...
$(document).ready(function() {
    ...
    $("a.logout").click(function() {
        $.ajax({
            type:"POST",
            url: HOME + "/logout",
            data:$(this).serialize() + '&redirectTo=/login',
            dataType: 'json',
            headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
            success: function(data, textStatus){
                toastr.success("Aguarde mientras cerramos la sesión.");
                location.href = HOME + data.redirectTo;
            },
            error: function(xhr, textStatus, errorThrown){
                toastr.error("Hubo un error. (" + textStatus + ")");
            }
        });
    });
